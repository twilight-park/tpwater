...
proc migrate-create { db table schema } {
	print sql db "create table $table ( [join $schema ,] )"
...
proc migrate-table { db table schema } {
    ...
    foreach column $inDB {
        print sql db "alter table $table drop column $column"
        sql db "alter table $table drop column $column"
    }
    foreach column $inSchema {
        print sql db "alter table $table add column [lsearch -inline -regex $schema $column]"
